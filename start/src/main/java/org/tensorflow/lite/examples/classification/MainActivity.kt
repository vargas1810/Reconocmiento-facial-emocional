...
import org.tensorflow.lite.examples.classification.ml.EmotionModel
...
class MainActivity : AppCompatActivity() {
    ...
    private class ImageAnalyzer(
        ...
        private val emotionModel = EmotionModel.newInstance(ctx, options)
        ...
        override fun analyze(imageProxy: ImageProxy) {
            ...
            // High-performance landmark detection and face classification
            ...
            detector.process(image!!)
                .addOnSuccessListener { faces ->
                    ...
                    for (face in faces) {
                        ...
                        Log.d("NUSRET","Left " + face.boundingBox.left.toString())
                        Log.d("NUSRET","top "+face.boundingBox.top.toString())
                        Log.d("NUSRET","Exact bla bla "+ (face.boundingBox.exactCenterY().toInt()/2).toString())
                        ...
                        //val scaled = Bitmap.createScaledBitmap(faceBitmap,160,160,true)

                        val my_image = (ctx as Activity).findViewById<ImageView>(R.id.my_face)
                        my_image.setImageBitmap(faceBitmap)

                        // TODO 2: Convert Image to Bitmap then to TensorImage
                        val tfImage = TensorImage.fromBitmap(faceBitmap)



                        // TODO 3: Process the image using the trained model, sort and pick out the top results
                        val outputs = emotionModel.process(tfImage)
                        ...
                        for (output in outputs) {
                            ...
                            val label = when (output.label) {
                                "happy" -> ctx.getString(R.string.happy)
                                "angry" -> ctx.getString(R.string.angry)
                                "disgust" -> ctx.getString(R.string.disgust)
                                "fear" -> ctx.getString(R.string.fear)
                                "surprise" -> ctx.getString(R.string.surprised)
                                "sad" -> ctx.getString(R.string.sad)
                                "neutral" -> ctx.getString(R.string.neutral)
                                else -> "Idk"
                            }
                            items.add(Recognition(label, output.score))
                        }
